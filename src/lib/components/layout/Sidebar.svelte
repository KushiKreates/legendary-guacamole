...
{#if $showSidebar}
	<div
		class=" fixed md:hidden z-40 top-0 right-0 left-0 bottom-0 bg-pink/60 w-full min-h-screen h-screen flex justify-center overflow-hidden overscroll-contain"
		on:mousedown={() => {
...
<div
	...
	class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
		? 'md:relative w-[260px]'
		: '-translate-x-[260px] w-[0px]'} pink-50 text-pink-900 dark:bg-pink-100 dark:text-pink-800 text-sm transition fixed z-50 top-0 left-0 rounded-r-2xl
        "
	...
	>
		<div class="px-2.5 flex justify-between space-x-1 text-gray-600 dark:text-gray-400">
			<a
				...
				class="flex flex-1 justify-between rounded-xl px-2 py-2 hover:bg-purple-200 dark:hover:bg-purple-250 transition"
				href="/"
				draggable="false"
				on:click={async () => {
					selectedChatId = null;
			...
			>
				<div class="self-center mx-1.5">
					<img
						...
						src="https://media.tenor.com/hYkRcm80JFwAAAAj/foxy-foxplushy.gif"
						class=" size-6 -translate-x-1.5 rounded-full"
						alt="logo"
					/>
				</div>
				<div class=" self-center font-medium text-sm text--850 dark:text-black">
					{$i18n.t('Astral Chat')}
				</div>
			...
			<button
				class=" cursor-pointer px-2 py-2 flex rounded-xl hover:bg-purple-100 dark:hover:bg-pink-850 transition"
				on:click={() => {
					showSidebar.set(!$showSidebar);
				}}
			>
		...
		{#if $user?.role === 'admin'}
			<div class="px-2.5 flex justify-center text-purple-600 dark:text-purple-900">
				<a
					class="flex-grow flex space-x-3 rounded-xl px-2.5 py-2 hover:bg-purple-200 dark:hover:bg-purple-200 transition"
					href="/workspace"
					on:click={() => {
						selectedChatId = null;
						chatId.set('');
				...
				>
					...
					<div class="flex self-center">
						<div class=" self-center font-medium text-sm">{$i18n.t('Admin Center')}</div>
					</div>
				</a>
			</div>
		{/if}
		...
		<div class="relative flex flex-col flex-1 overflow-y-auto">
			{#if !($settings.saveChatHistory ?? true)}
				<div class="absolute z-40 w-full h-full bg-purple-250/290 dark:bg-pink/190 flex justify-center">
			...
			<div class="px-2 mt-0.5 mb-2 flex justify-center space-x-2">
				<div class="flex w-full rounded-xl" id="chat-search">
					<div class="self-center pl-3 py-2 rounded-l-xl bg-purple-200">
						<svg
							xmlns="http://www.w3.org/2000/svg"
							viewBox="0 0 20 20"
							fill="currentColor"
					...
					<input
						class="w-full rounded-r-xl py-1.5 pl-2.5 pr-4 text-purple-100 bg-purple-200 dark:text-black-900 outline-none"
			...
			{#if $tags.length > 0}
				<div class="px-2.5 mb-2 flex gap-1 flex-wrap">
					<button
						class="px-2.5 text-xs font-medium bg-purple-350 dark:bg-purple-900 dark:hover:bg-purple-800 transition rounded-full"
						on:click={async () => {
							await chats.set(await getChatList(localStorage.token));
						}}
					>
					...
					{#each $tags as tag}
						<button
							class="px-2.5 text-xs font-medium bg-purple-600 dark:bg-purple-900 dark:hover:bg-purple-700 transition rounded-full"
							on:click={async () => {
			...
			<div class="pl-2 my-2 flex-1 flex flex-col space-y-1 overflow-y-auto scrollbar-hidden">
				{#each filteredChatList as chat, idx}
					{#if idx === 0 || (idx > 0 && chat.time_range !== filteredChatList[idx - 1].time_range)}
						<div
							class="w-full pl-2.5 text-xs text-purple-900 dark:text-purple-740 font-medium {idx === 0
								? ''
								: 'pt-5'} pb-0.5"
						>
		...
		<div class="px-2.5">
			...
			<div class="flex flex-col">
				{#if $user !== undefined}
					...
					>
						<button
							class=" flex rounded-xl py-3 px-3.5 w-full hover:bg-purple-900 dark:hover:bg-purple-900 transition"
							on:click={() => {
						...
						>
							...
							<div class=" self-center font-semibold">{$user.name}-chan</div>
						</button>
					</UserMenu>
				{/if}
			</div>
