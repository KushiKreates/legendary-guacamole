	...
	{#if confirmEdit}
		<div
			class=" w-full flex justify-between rounded-xl px-3 py-2 {chat.id === $chatId || confirmEdit
				...
				: selected
				? 'bg-pink-100 dark:bg-gray-950'
				: 'group-hover:bg-purple-900 dark:group-hover:bg-gray-950'}  whitespace-nowrap text-ellipsis"
		>
			<input
				use:focusEdit
	...
	{:else}
		...
		>
			<div class=" flex self-center flex-1 w-full">
				<div class=" text-left self-center overflow-hidden w-full h-[20px] dark:text-pink-900">
					{chat.title}
				</div>
			</div>
		</a>
			...
			<div class="flex self-center space-x-1 z-10">
				...
				>
					<button
						...
						class=" self-center hover:text-white transition dark:hover:text-white transition"
						on:click={() => {
							dispatch('select');
						}}
					>
