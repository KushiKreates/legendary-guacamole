...
{#if prompts.length > 0}
	<div class="mb-2 flex gap-1 text-sm font-medium items-center text-gray-400 dark:text-purple-900">
		<Bolt />
		{$i18n.t('Suggested')}
	</div>
{/if}
				...
				>
					<div class="flex flex-col text-left">
						{#if prompt.title && prompt.title[0] !== ''}
							<div
								class="  font-medium dark:text-pink-300 dark:group-hover:text-gray-200 transition"
							>
								{prompt.title[0]}
							</div>
							<div class="text-sm text-purple-600 font-normal line-clamp-2">{prompt.title[1]}</div>
						{:else}
							<div
								class="  text-sm font-medium dark:text-gray-300 dark:group-hover:text-pink-500 transition line-clamp-2"
							>
								{prompt.content}
							</div>
						{/if}
