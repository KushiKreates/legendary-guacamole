...
<div class="w-full">
	...
	<div class="{transparentBackground ? 'bg-transparent' : 'bg-pink-100 dark:bg-purple-400'} ">
		<div class="max-w-6xl px-2.5 md:px-6 mx-auto inset-x-0">
			<div class=" pb-2">
					...
					on:change={async () => {
						if (inputFiles && inputFiles.length > 0) {
							...
							_inputFiles.forEach((file) => {
								if (['image/gif', 'image/webp', 'image/jpeg', 'image/png'].includes(file['type'])) {
									if (visionCapableModels.length === 0) {
										toast.error($i18n.t('Selected model(s) do not support image inputs, Please be kind enough to use jpg, webp and png.'));
										return;
									}
									let reader = new FileReader();
									reader.onload = (event) => {
					...
					>
							...
							<div class=" flex">
								...
								<div class="self-end mb-2 flex space-x-1 mr-1">
									{#if messages.length == 0 || messages.at(-1).done == true}
										<Tooltip content={$i18n.t('Talk Engine')}>
											<button
						...
						<div class="flex items-end w-10">
							{#if messages.length == 0 || messages.at(-1).done == true}
								...
								{:else}
									<div class=" flex items-center mb-1">
										<Tooltip content={$i18n.t('Talk to it!')}>
											<button
												id="send-message-button"
												class="{prompt !== ''
													? 'bg-black text-white hover:bg-gray-900 dark:bg-pink-100 dark:text-black dark:hover:bg-gray-100 '
							...
							{:else}
								<div class=" flex items-center mb-1.5">
									<button
										class="bg-pink-100 hover:bg-pink-100 text-gray-800 dark:bg-pink-700 dark:text-white dark:hover:bg-pink-800 transition rounded-full p-1.5"
										on:click={() => {
											stopResponse();
										}}
									>
				...
				<div class="mt-1.5 text-xs text-white-900 text-center line-clamp-1">
					{$i18n.t('This platform is Free for all Disussions, Your models may make mistakes!')}
				</div>
			</div>
		</div>
	</div>
