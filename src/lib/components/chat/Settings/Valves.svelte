	...
	<div class="flex flex-col pr-1.5 overflow-y-scroll max-h-[25rem]">
			...
			<div class="flex items-center justify-between mb-2">
				<Tooltip content="">
					<div class="text-sm font-medium">
						{$i18n.t('Manage Valves (Don`t mess around if you don`t know 🔓)')}
					</div>
				</Tooltip>

				<div class=" self-end">
					<select
						class=" dark:bg-purple-200 w-fit pr-8 rounded text-xs bg-transparent outline-none text-right"
						bind:value={tab}
						placeholder="Select"
					>
		...
		<div class="space-y-1">
			<div class="flex gap-2">
				<div class="flex-1">
					<select
						class="w-full rounded-lg py-2 px-4 dark:text-pink-900 dark:bg-green-850 outline-none"
					...
					>
						{#if tab === 'tools'}
							<option value="" selected disabled class="bg-gray-100 dark:bg-pink-700"
								>{$i18n.t('Select a tool')}</option
							>

							{#each $tools as tool, toolIdx}
								<option value={tool.id} class="bg-gray-100 dark:bg-pink-700">{tool.name}</option>
							{/each}
						{:else if tab === 'functions'}
